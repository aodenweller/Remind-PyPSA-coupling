"""SSP population & GDP proxy datasets used to weight downscaling.
...
def fetch_ssp_variable(
    ...
    *,
    ...
    Args:
        variable: IAMC variable name (e.g. ``"Population"``).
        model: IIASA model name filter.
        scenario: IIASA scenario name filter.
        url: API endpoint.
        page_size: Rows requested per page.
        timeout: Per-request timeout, in seconds.

    Returns:
        ``[iso2, year, value]``, summed over duplicate ``(iso2, year)`` rows and with
        unmapped regions dropped.
    """
...
def fetch_ssp_data(
    ...
    Args:
        scenario: IIASA scenario name filter, shared by both variables.
        population_model: IIASA model name filter for the ``Population`` variable.
        gdp_model: IIASA model name filter for the ``GDP|PPP`` variable.

    Returns:
        ``(population, gdp)``, each ``[iso2, year, value]``.
    """
...
def read_ssp_data(
    ...
    Args:
        path: Snapshot CSV to read.
        variables: Accepted for API symmetry with the fetch functions; unused, since a
            snapshot is single-variable.
